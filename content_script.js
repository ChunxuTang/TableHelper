...
style.type = 'text/css';
style.href = chrome.extension.getURL('content_script.css');
(document.head||document.documentElement).appendChild(style);


// Parts of the code below are from Chrome extension Copytables
...
var $A = function (a, coll) {
  Array.prototype.forEach.call(coll, function (x) {
    a.push(x)
  });
  return a;
};
...
var selectionInit = function (el, extend) {
  ...
  // I want to try whether I could find the index of the table in tables
  // var tables = document.getElementsByTagName("table");
  // // console.log(tables);
  // var table_index = Array.prototype.indexOf.call(tables, table);
...
var contentForCopy = function (command) {
  ...
  switch (command) {
    case "copyHTML":
      // currently not to copy css styles.
...
var doCopy = function (command) {
  ...
  // added
  // To record the index of selected and copied table.
  var tables = document.getElementsByTagName("table");
  // console.log(tables);
  var table_index = Array.prototype.indexOf.call(tables, selection.table);
  console.log('table_index: ' + table_index);

  chrome.runtime.sendMessage({command: command, content: contentForCopy(command), index: table_index});
};
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  switch (message.command) {
    ...
    case "dropTable":
      var content;
      chrome.storage.local.get('table', function (res) {
        content = res.table;
        ...
        chrome.storage.local.get('url', function(res) {
          source.innerHTML = "Source: " + res.url;
          div.appendChild(source);
          document.body.appendChild(div);
          page_url = res.url;
          chrome.storage.local.get('index', function(res) {
            // Post message to target web page. The target web page needs to handle the message,
            // such as appending the table to the end of the web page.
            window.postMessage({ type: "FROM_PAGE", text: div.innerHTML, url: page_url, index: res.index}, "*");
          });
